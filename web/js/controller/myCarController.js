$(function(){
	function success(data){
		...
		for (var i=0;i<=data.EquipmentDailyStatus.length-1;i++) {
			for (var prop in data.EquipmentDailyStatus[i]) {
				if (!data.EquipmentDailyStatus[i][prop]||data.EquipmentDailyStatus[i][prop] == null) {
					data.EquipmentDailyStatus[i][prop] = ""
				}
			}
			...
			$('ul li:first-child .myCarInput').eq(0).text(data.EquipmentDailyStatus[i].EquipmentWorkStatusName);
